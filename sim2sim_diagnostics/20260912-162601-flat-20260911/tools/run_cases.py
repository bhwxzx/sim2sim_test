...
directory=out/'cases'/'reset-check';directory.mkdir()
(directory/'result.json').write_text(json.dumps(plan['reset_check'],indent=2)+'\n')
for spec in commands:
    ...
    with (folder/'console.log').open('x') as log:
        try:
            ...
            code=124
            if not (folder/'result.json').exists():(folder/'result.json').write_text(json.dumps({'case_id':spec['case_id'],'status':'failed','termination_reason':'supervisor_wall_timeout','requested_sim_duration':20,'final_sim_time':None,'control_records':None})+'\n')
