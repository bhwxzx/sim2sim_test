...
struct Diagnostic : RL {
    ...
    J state() {
        ...
        J j={{"sim_time",d->time},{"qpos_mujoco",arr(d->qpos,m->nq)},{"qvel_mujoco",arr(d->qvel,m->nv)},
            ...
            {"base_linear_velocity_world",arr(vw+3,3)},{"base_linear_velocity_body",arr(vb+3,3)},
            {"base_angular_velocity_world",arr(vw,3)},{"base_angular_velocity_body",arr(vb,3)},
            {"projected_gravity_from_raw_pose",pg},{"pitch_ZYX_rad",pitch},{"backward_lean_deg",-pitch*180/M_PI},{"roll_ZYX_deg",roll*180/M_PI},
...
int main(int argc,char**argv) {
    if(argc!=4){std::cerr<<"usage: collect XML CASE_ID OUTPUT_DIR\n";return 2;}
    ...
    try {
        ...
        while(h.d->time<duration-1e-10) {
            ...
            J row={{"case_id",cid},{"control_step",step},{"simulation_time",h.d->time},
            ...
            row["safety_events"]=h.safety_events;emit(row);++step;
