...
private:
    bool runForwardIntoActions()
    {
        ...
        // Approved diagnostic-only hook: each frame before original history/reset insertion.
        extern void LWDiagnosticNoise(const LWPolicyRuntimeConfiguration&, const Observations<float>&, std::vector<float>&);
        LWDiagnosticNoise(policy_configuration, inference_obs_, inference_flat_obs_);
