...
cmd=['/usr/bin/c++','-std=c++17','-O2','-DUSE_ONNX','-DPOLICY_DIR=""',f'-I{source.parent}',*includes,*map(str,sources),*[f'-L{x}' for x in libdirs],'-Wl,-rpath,'+':'.join(map(str,libdirs)),'-lmujoco','-lonnxruntime','-lyaml-cpp','-ltbb','-lz','-lpthread','-o',str(a.output/'collect')]
